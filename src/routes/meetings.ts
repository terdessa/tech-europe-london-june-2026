...
import { deleteMeeting, listMeetings } from "../db.js";
...
// DELETE /meetings/:meetingId — wipe all stored context so the id can be reused.
export const handleDeleteMeeting = (req: Request, res: Response): void => {
  ...
  try {
    ...
    res.json({ ok: true, meetingId, deleted });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    sendError(res, 500, msg);
  }
